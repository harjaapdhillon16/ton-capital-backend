-- TON Capital AI - Supabase initial schema bootstrap
-- Run this once in Supabase SQL Editor for a fresh environment.
...
create index if not exists idx_bot_conversations_user_chat_created on bot_conversations(user_id, telegram_chat_id, created_at desc);
create index if not exists idx_bot_conversations_user_created on bot_conversations(user_id, created_at desc);

-- Optional: enable RLS and add read policies (for anon/auth clients)
-- Keep disabled if only service role accesses these tables.
--
